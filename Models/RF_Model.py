...
for train, test in kf.split(diabetes):
    ...
    np_training_acc.append(rf.score(X_train, y_train))
    np_testing_acc.append(rf.score(X_test, y_test))
    np_feature_importance.append(rf.feature_importances_)

print('Accuracy of non-private RF classifier on training set: {:.2f}'.format(np.mean(np_training_acc)))
print('Accuracy of non-private RF classifier on test set: {:.2f}'.format(np.mean(np_testing_acc)))
...
for i in range(200):
    ...
    for train, test in kf.split(diabetes):
        ...
        testing_acc.append(rf.score(X_test, y_test))
        l_feature_importance.append(rf.feature_importances_)

    l_training_acc.append(np.mean(training_acc))
    l_testing_acc.append(np.mean(testing_acc))
    
print('Accuracy of laplace private RF classifier on training set: {:.2f}'.format(np.mean(l_training_acc)))
print('Accuracy of laplace private RF classifier on test set: {:.2f}'.format(np.mean(l_testing_acc)))
...
plt.title("RF Training Set Accuracy under Laplace Noise")
plt.xlabel("Accuracy (%)")
...
plt.title("RF Testing Set Accuracy under Laplace Noise")
plt.xlabel("Accuracy (%)")
plt.ylabel("Number of Samples")
plt.show()
...
for i in range(200):
    ...
    for train, test in kf.split(diabetes):
        ...
        training_acc.append(rf.score(X_train, y_train))
        testing_acc.append(rf.score(X_test, y_test))
        g_feature_importance.append(rf.feature_importances_)

    g_training_acc.append(np.mean(training_acc))
    g_testing_acc.append(np.mean(testing_acc))
    
print('Accuracy of gaussian private RF classifier on training set: {:.2f}'.format(np.mean(g_training_acc)))
print('Accuracy of gaussian private RF classifier on test set: {:.2f}'.format(np.mean(g_testing_acc)))

fig, axis = plt.subplots(figsize =(10, 5))
plt.hist(l_training_acc, bins = np.linspace(np.amin(g_training_acc), np.amax(np_training_acc), 20))
plt.axvline(np.mean(np_training_acc), color='k', linestyle='dashed', linewidth=1)
plt.title("RF Training Set Accuracy under Gaussian Noise")
plt.xlabel("Accuracy (%)")
plt.ylabel("Number of Samples")
plt.show()
...
plt.axvline(np.mean(np_testing_acc), color='k', linestyle='dashed', linewidth=1)
plt.title("RF Testing Set Accuracy under Gaussian Noise")
plt.xlabel("Accuracy (%)")
plt.ylabel("Number of Samples")
plt.show()
